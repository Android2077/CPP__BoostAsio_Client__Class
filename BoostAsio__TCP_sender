...
#include <boost\asio.hpp>      //[подключаем заголовнчый файл boost asio]




class BoostAsio__TCP_sender
{
...
public:
	...
	enum class RecieveBuffer_flag : int
	{
		Internal_buffer = 1,
		User_buffer = 2,
	};
...
public:
	...
	public:
		...
		struct socket_struct
		{
			...
			struct Write_struct
			{
				...
				std::deque<std::function<void()>>deque_for_Sequential_Send;    //Вообщем суть такая: Когда я вызваю отправку сообщений на удаленный IP к примеру пусть это будет на для глядности какие-то данные в 1Гб, и за ними сразу же какие то другие данные размер которых уже не важен, то Boost Asio Не гарантирует, что второе сообщение отрпавится строго только после того, как отправятся все байты первого сообщения, поэтому по факту произодйет перемешевание байт первого и второго сообщения при отправке. ПОЭТОМУ нужно синхронизировать последовательность вызовов отправки сообщений, ТО ЕСТЬ ждать пока порлность первое сообщение не отправится и только потом вызывать отправку второго сообщение поместив его в орчедь на ожидание, и так далее. А мьютекс тут нужен для синхронизации проверки размер очерди ожидания "deque_task" и флага "Full_Transfered__flag" и самое главное для синхронизации вызова ассинхронных колбеков "async_write_some" - так колбек "async_write_some" будет вызыватся в любом случае из других внутренних потоков Boost Asio, чем вызовы метода данного класса на отправку сообщения. Так что без мьютекса не обойтись. НО ВНИМАНИЕ!!! ЭТО АКТУАЛЬНО ТОЛЬКО ЕСЛИ КЛАСС СОЗДАН С ПАРАМЕТРОМ "multi_threads", если класс создан с параметром "main_thread" - то все асихнронные колбеки вызываются в том же потоке в котором создан обькт данного класса и в данном слчае выполнение будетстрого последовтаельным и защита мьюектса для дсотупа к указанной очерди и флагу - НЕ НУЖНА!!! Возможно я разделю класс на два типа: main_thread и Othe_threads - чтобы, если класс создается с параметром main_thread - мьютекс просто так не снижал производительность.
				//-----------------------------------------------------------------------------------------------------------------
			...
			struct Read_struct
			{
				...
				Read_struct()
				{
					BufferReceiver.resize(BufferReceiver_UserSize);  //Размер по умолчанию.

					pointer_to_used_buffer = &BufferReceiver[0];
					buffer_size_used = BufferReceiver_UserSize;
				}

				void set__InternalBuffer(const size_t size)
				{
					BufferReceiver.resize(size);

					pointer_to_used_buffer = &BufferReceiver[0];
					buffer_size_used = size;
				}

				void set__UserBuffer(char* pointer_to_UserBuffer, const size_t size)
				{
					pointer_to_used_buffer = &BufferReceiver[0];
					buffer_size_used = size;
				}
				...
				//------------------------------------
				char* pointer_to_used_buffer = 0;
				size_t buffer_size_used = 0;
				//------------------------------------

				//-------------------------------------
				std::string BufferReceiver;
				size_t BufferReceiver_UserSize = 1024;     //Размер приемного буффера по умолчанию. 
				//-------------------------------------
			...
			//-------------------Для механизма контроля отправки и приема сообщений, только после успешного Connect c удаленным IP:----------------------------------------------
			bool Connect_flag = false;

			std::deque<std::function<void()>>deque__Tasks_waiting_for_Connection;
			//-------------------Для механизма контроля отправки и приема сообщений, только после успешного Connect c удаленным IP:----------------------------------------------


			//**********************************Расширяемые данные:Конец***************************************************
			...
			socket_struct(const std::string& Socket_name_, boost::asio::ip::tcp IP_type_, boost::asio::ip::tcp::socket&& socket__, const std::string& web_adress_) : Socket_name(Socket_name_), IP_type(IP_type_), socket_(std::move(socket__)), web_adress(web_adress_) {}
		};
	...
	public:
		...
		socket_struct* get__OpenSocket_by_Name(const std::string& SocektName)
		{

			std::unordered_map<std::string, socket_struct*>::iterator it_find = UnorderedMap_SocketName.find(SocektName);

			if (it_find != UnorderedMap_SocketName.end())
			{
				if (it_find->second->socket_.is_open() == true)
				{
		...
		socket_struct* get__Socket_by_Name(const std::string& SocektName)
		{
	...
	public:
		...
		socket_struct* add__NewSocket(const std::string& Socket_name, const boost::asio::ip::tcp IP_type)
		{
			...
			socket.open(IP_type, boost_error_code);

			if (boost_error_code.value() != boost::system::errc::success)
			{
				lambda_error(this, 0, error_enum::boost_error, boost_error_code, "add__NewSocket:open():" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
			...
			{
				//--------------------------------------------------------------------------
				list_socket.emplace_back(Socket_name, IP_type, std::move(socket));
		...
		socket_struct* open_Previously_ClosedSocket(const boost::asio::ip::tcp IP_type)
		{
			...
			if (deque_SocketsClosed.size() > 0)
			{
				...
				if (boost_error_code.value() != boost::system::errc::success)
				{
					lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error_code, "open_Previously_ClosedSocket:open():" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
				...
				//-----------------------------------------------------------
				socket_struct_p->Write_struct_.bytes_send_transferred = 0;                   //Сбрасываем счетчик.
				socket_struct_p->Connect_flag = false;
				//-----------------------------------------------------------
	...
	private:
		...
		std::unordered_map<std::string, socket_struct*> UnorderedMap_SocketName;
...
public:
	...
	BoostAsio__TCP_sender(const ThreadType_enum ThreadType_enum_, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const error_enum error_enum_, const boost::system::error_code& boost_error, const std::string& error_string)> lambda_error_)
	{
		Sockets__class_uptr.reset(new Sockets__class(&io_context, [this, lambda_error_](Sockets__class* Sockets__class_p, Sockets__class::socket_struct* socket_struct_p, const error_enum error_enum_, const boost::system::error_code& boost_error, const std::string& text_error)
			{
				lambda_error_(this, socket_struct_p, error_enum_, boost_error, text_error);
			}));

		work_guard_uptr.reset(new boost::asio::executor_work_guard<boost::asio::io_context::executor_type>(io_context.get_executor()));

		resolver_uptr.reset(new boost::asio::ip::tcp::resolver(io_context));
		...
		if (ThreadType_enum_ == ThreadType_enum::multi_threads)
		{
			...
			{
				vec_thread[i] = std::thread(&BoostAsio__TCP_sender::io_context__Run, this);
			}
			//------------------------------------------------------------------
		}
		...
		ThreadPoolTask__uptr.get()->set_lambda_error([this](const ThreadPoolTask__class::error_enum error_enum_, const std::string error)
			{
				...
				{
					lambda_error(this, 0, error_enum::bad_alloc, boost_error_code, "ThreadPoolTask:" + error);
				}
				else
				{
					if (error_enum_ == ThreadPoolTask__class::error_enum::other_error)
					{
						lambda_error(this, 0, error_enum::other_error, boost_error_code, "ThreadPoolTask:" + error);
					}
				}
		...
		ThreadPoolTask__uptr.get()->add_task([this]()
			{
				...
				resolver_uptr.get()->cancel();

				Sockets__class_uptr.get()->close_AllSocket();          //Отменяем все незавершенные асинхроные операции.

				work_guard_uptr.get()->reset();

				Sockets__class_uptr.get()->delete_AllSocket();
...
public:
	...
	void set__Callback_Error(std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const error_enum error_enum_, const boost::system::error_code& boost_error, const std::string& error_string)> user_lambda_)
	{
	...
	void set__Callback_for_Connect(std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error) > user_lambda_)
	{
		ThreadPoolTask__uptr.get()->add_task([this, user_lambda_]()
			{
				Shared_User_Callback_AsyncConnect = user_lambda_;
			});

	}

	void set__Callback_for_SocketWrite(std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender__class_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> user_lambda_)
	{
		ThreadPoolTask__uptr.get()->add_task([this, user_lambda_]()
			{
				Shared_User_Callback_for_Send = user_lambda_;
			});

	}

	void set__Callback_for_SocketRead(std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender__class_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> user_lambda_)
	{
		ThreadPoolTask__uptr.get()->add_task([this, user_lambda_]()
			{
				Shared_User_Callback_for_Read = user_lambda_;
			});

	}

	void set__CallbackMode_for_SocketWrite(Sockets__class::socket_struct* socket_struct_p, const CallbackMode CallbackMode_)
	{
		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, CallbackMode_]()
			{
				socket_struct_p->Write_struct_.CallbackMode_ = CallbackMode_;
			});
	...
	void Connect_to_IP_adress(Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_IP, const int Port_num, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error)> User_Callback_AsyncConnect)
	{

		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, pointer_to_IP, Port_num, User_Callback_AsyncConnect]()
			{


				socket_struct_p->web_adress = "";


				//-------------------------------------------------------------------
				if (socket_struct_p->socket_.is_open() == false)
				{
					const bool res = Sockets__class_uptr.get()->redefine_socket(socket_struct_p, socket_struct_p->IP_type);
				...
				//-------------------------------------------------------------------
				call__async_connect(socket_struct_p, pointer_to_IP, Port_num, User_Callback_AsyncConnect);
				//-------------------------------------------------------------------
	...
	void Connect_to_Web_adress(Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_Web_Adress, const char* Port_num, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error)> User_Callback_AsyncConnect)
	{

		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, pointer_to_Web_Adress, Port_num, User_Callback_AsyncConnect]()
			{


				socket_struct_p->web_adress = pointer_to_Web_Adress;
				...
				AsyncResolver_handler(socket_struct_p, pointer_to_Web_Adress, Port_num, User_Callback_AsyncConnect);

			});
	}

	void send_to_socket(Sockets__class::socket_struct* socket_struct_p, char* pointer_to_buff, const size_t buff_size, const bool data_copy_flag, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> User_Callback_for_Send)
	{
		...
		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, pointer_to_buff_, buff_size, User_Callback_for_Send]()
			{



				//----------------------------------------------------------------------------------------------------------------------------------------------
				if (socket_struct_p->Connect_flag == true)
				{
					//Значит соединение на сокете в данный момент установлено:

					send_to_socket__(socket_struct_p, pointer_to_buff_, buff_size, User_Callback_for_Send);
				}
				else
				{
					//Значит соединение на сокете в данный момент НЕ установлено:

					socket_struct_p->deque__Tasks_waiting_for_Connection.push_back([this, socket_struct_p, pointer_to_buff_, buff_size, User_Callback_for_Send]()
						{
							send_to_socket__(socket_struct_p, pointer_to_buff_, buff_size, User_Callback_for_Send);
						});
				}
	...
	void set__Read(Sockets__class::socket_struct* socket_struct_p, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> User_Callback_for_Read)
	{


		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, User_Callback_for_Read]()
			{
				...
				if (socket_struct_p->Connect_flag == true)
				{
					...
					AsyncRead_handler(socket_struct_p, User_Callback_for_Read);
				}
				else
				{
					//Значит соединение на сокете в данный момент НЕ установлено:

					socket_struct_p->deque__Tasks_waiting_for_Connection.push_back([this, socket_struct_p, User_Callback_for_Read]()
						{
							AsyncRead_handler(socket_struct_p, User_Callback_for_Read);
						});
				}
	...
	void set__InternalBuffer_for_Read(Sockets__class::socket_struct* socket_struct_p, const size_t buff_size)
	{

		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, buff_size]()
			{
				socket_struct_p->Read_struct_.set__InternalBuffer(buff_size);
			});

	}
	void set__UserBuffer_for_Read(Sockets__class::socket_struct* socket_struct_p, char* pointer_to_UserBuffer, const size_t buff_size)
	{
		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, pointer_to_UserBuffer, buff_size]()
			{
				socket_struct_p->Read_struct_.set__UserBuffer(pointer_to_UserBuffer, buff_size);
			});

	}

	void set__ReadUntil(Sockets__class::socket_struct* socket_struct_p, const std::string& user_separator, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> User_Callback_for_Read)
	{

		ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, user_separator, User_Callback_for_Read]()
			{

				//----------------------------------------------------------------------------
				socket_struct_p->Read_struct_.read_untill_.user_separator = user_separator;
				//----------------------------------------------------------------------------

				AsyncReadUntill_handler(socket_struct_p, User_Callback_for_Read);
	...
	Sockets__class::socket_struct* get__Socket_by_Name(const std::string& Socket_name)
	{
		return get__SocketClass_pointer()->get__Socket_by_Name(Socket_name);
	}
...
private:
	...
	std::unique_ptr<boost::asio::ip::tcp::resolver> resolver_uptr;
	//----------------------------------------------------------
	...
	//-------------------для механизма последовательности выполнения задач отправки и общей потоко-безопасности:----------------------------------------------
	//std::mutex mutex_;         //Вообщем суть такая: Когда я вызваю отправку сообщений на удаленный IP к примеру пусть это будет на для глядности какие-то данные в 1Гб, и за ними сразу же какие то другие данные размер которых уже не важен, то Boost Asio Не гарантирует, что второе сообщение отрпавится строго только после того, как отправятся все байты первого сообщения, поэтому по факту произодйет перемешевание байт первого и второго сообщения при отправке. ПОЭТОМУ нужно синхронизировать последовательность вызовов отправки сообщений, ТО ЕСТЬ ждать пока порлность первое сообщение не отправится и только потом вызывать отправку второго сообщение поместив его в орчедь на ожидание, и так далее. А мьютекс тут нужен для синхронизации проверки размер очерди ожидания "deque_task" и флага "Full_Transfered__flag" и самое главное для синхронизации вызова ассинхронных колбеков "async_write_some" - так колбек "async_write_some" будет вызыватся в любом случае из других внутренних потоков Boost Asio, чем вызовы метода данного класса на отправку сообщения. Так что без мьютекса не обойтись. НО ВНИМАНИЕ!!! ЭТО АКТУАЛЬНО ТОЛЬКО ЕСЛИ КЛАСС СОЗДАН С ПАРАМЕТРОМ "multi_threads", если класс создан с параметром "main_thread" - то все асихнронные колбеки вызываются в том же потоке в котором создан обькт данного класса и в данном слчае выполнение будетстрого последовтаельным и защита мьюектса для дсотупа к указанной очерди и флагу - НЕ НУЖНА!!! Возможно я разделю класс на два типа: main_thread и Othe_threads - чтобы, если класс создается с параметром main_thread - мьютекс просто так не снижал производительность.
	...
	//--------------------------------------------------------------
	std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const error_enum error_enum_, const boost::system::error_code& boost_error, const std::string& error_string)>lambda_error;
	//--------------------------------------------------------------
	std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> Shared_User_Callback_for_Read = 0;
	//--------------------------------------------------------------
	std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> Shared_User_Callback_for_Send = 0;
	//--------------------------------------------------------------
	std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error)> Shared_User_Callback_AsyncConnect = 0;
	//--------------------------------------------------------------
	...
	void send_to_socket_(Sockets__class::socket_struct* socket_struct_p, char* pointer_to_buff, const size_t buff_size, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> User_Callback_for_Send)
	{
		...
		AsyncWrite_handler(socket_struct_p, User_Callback_for_Send);

	}
	void send_to_socket__(Sockets__class::socket_struct* socket_struct_p, char* pointer_to_buff, const size_t buff_size, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> User_Callback_for_Send)
	{
		//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		if (socket_struct_p->Write_struct_.Full_Transfered__flag == false)
		{
			...
			socket_struct_p->Write_struct_.deque_for_Sequential_Send.push_back([this, socket_struct_p, pointer_to_buff, buff_size, User_Callback_for_Send]()
				{
					//Добавим данный запрос в очередь на ожидание, пока все данные по действующему запросу не будут отправлены.

					send_to_socket_(socket_struct_p, pointer_to_buff, buff_size, User_Callback_for_Send);
				});
	...
	inline void AsyncWrite_handler(Sockets__class::socket_struct* socket_struct_p, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const CallbackMessage CallbackMessage_, const size_t bytes_send_transferred, const size_t bytes_send_transferred_TOTAL)> User_Callback_for_Send)
	{
		...
		socket_struct_p->socket_.async_write_some(buffer_, [this, socket_struct_p, User_Callback_for_Send](const boost::system::error_code& boost_error, const std::size_t bytes_send_transferred)
			{
				...
				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error, bytes_send_transferred, User_Callback_for_Send]()
					{
						...
						if (boost_error.value() != boost::system::errc::success)
						{
							...
							if (boost_error.value() != boost::asio::error::operation_aborted)        //operation_aborted - означает, что задача была отменена вызовом метода "cancel" у обьекта "resolver_uptr" - поэтому отфильтруем эти "ошибки" - так как на самом деле они не ошбики.
							{
								...
								//--------------------------------------------------
								const bool res = CheckSocket_on_Disconnect(socket_struct_p, boost_error);    //Проверим соответвует ли пришедшая ошибка - разорванному соединению. Если возвращает "true" - то значит, эта "ошибка" соовтетвует закртию соединения и не нужно ее расмтаривать, как ошибку и вызывать "lambda_error".
								//--------------------------------------------------

								//--------------------------------------------------
								if (res == false)
								{
									lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error, "AsyncWrite_handler:async_write_some:" + get__BoostErrorText(boost_error.value()) + ":" + boost_error.message());
								}
							    //--------------------------------------------------
							}
						...
						else
						{
							...
							if (socket_struct_p->Write_struct_.bytes_send_transferred == socket_struct_p->Write_struct_.UserBuff_struct_.Buff_size)
							{
								...
								//------------------------------------------------------
								if (Shared_User_Callback_for_Send != 0)
								{
									Shared_User_Callback_for_Send(this, socket_struct_p, CallbackMessage::Full, bytes_send_transferred, socket_struct_p->Write_struct_.bytes_send_transferred);
								}

								if (User_Callback_for_Send != 0)
								{
									User_Callback_for_Send(this, socket_struct_p, CallbackMessage::Full, bytes_send_transferred, socket_struct_p->Write_struct_.bytes_send_transferred);
								}
								//------------------------------------------------------
							...
							else
							{
								...
								if (socket_struct_p->Write_struct_.CallbackMode_ == CallbackMode::Call_AlwaysPart_Send)
								{
									if (Shared_User_Callback_for_Send != 0)
									{
										Shared_User_Callback_for_Send(this, socket_struct_p, CallbackMessage::Part, bytes_send_transferred, socket_struct_p->Write_struct_.bytes_send_transferred);
									}

									if (User_Callback_for_Send != 0)
									{
										User_Callback_for_Send(this, socket_struct_p, CallbackMessage::Part, bytes_send_transferred, socket_struct_p->Write_struct_.bytes_send_transferred);
									}
								}

								AsyncWrite_handler(socket_struct_p, Shared_User_Callback_for_Send);        //Так как еще не все данные передалались, то вызываем функцию "AsyncWrite_handler" до тех пор, пока все данные не передадутся, так как функция "async_write_some" вызывает колбек толко один раз, поэтому нужно вручную вызывать.
	...
	inline void AsyncRead_handler(Sockets__class::socket_struct* socket_struct_p, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> User_Callback_for_Read)
	{

		socket_struct_p->socket_.async_read_some(boost::asio::buffer(socket_struct_p->Read_struct_.pointer_to_used_buffer, socket_struct_p->Read_struct_.buffer_size_used), [this, socket_struct_p, User_Callback_for_Read](const boost::system::error_code& boost_error_code, const std::size_t data_size)
			{
				//Данный лямбда кол-бек ВСЕГДА будет вызываться в том потоке в котором запущен данных обьект класса(при установке при создании класса параметра main_thread) или в нескольких отдельных внутренних потоках (при установке при создании класса параметра othre_threads) Boost Asio - отличных от любого ПОльзовательского потока.


				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error_code, data_size, User_Callback_for_Read]()
					{
						...
						if (boost_error_code.value() != boost::system::errc::success)
						{
							...
							{
								//--------------------------------------------------
								const bool res = CheckSocket_on_Disconnect(socket_struct_p, boost_error_code);    //Проверим соответвует ли пришедшая ошибка - разорванному соединению. Если возвращает "true" - то значит, эта "ошибка" соовтетвует закртию соединения и не нужно ее расмтаривать, как ошибку и вызывать "lambda_error".
								//--------------------------------------------------

								//--------------------------------------------------
								if (res == false)
								{
									lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error_code, "AsyncRead_handler:async_read_some:" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
								}
								//--------------------------------------------------
							}
						}
						else
						{
							...
							if (Shared_User_Callback_for_Read != 0)
							{
								Shared_User_Callback_for_Read(this, socket_struct_p, socket_struct_p->Read_struct_.BufferReceiver.c_str(), data_size);
							}

							if (User_Callback_for_Read != 0)
							{
								User_Callback_for_Read(this, socket_struct_p, socket_struct_p->Read_struct_.BufferReceiver.c_str(), data_size);
							}
	...
	inline void AsyncReadUntill_handler(Sockets__class::socket_struct* socket_struct_p, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_DataReceived, const size_t bytes_received)> User_Callback_for_Read)
	{

		boost::asio::async_read_until(socket_struct_p->socket_, boost::asio::dynamic_buffer(socket_struct_p->Read_struct_.read_untill_.string_buff), socket_struct_p->Read_struct_.read_untill_.user_separator.c_str(), [this, socket_struct_p, User_Callback_for_Read](const boost::system::error_code& boost_error_code, const std::size_t data_size)
			{
				//Данный лямбда кол-бек ВСЕГДА будет вызываться в том потоке в котором запущен данных обьект класса(при установке при создании класса параметра main_thread) или в нескольких отдельных внутренних потоках (при установке при создании класса параметра othre_threads) Boost Asio - отличных от любого ПОльзовательского потока.
				...
				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error_code, data_size, User_Callback_for_Read]()
					{
						...
						if (boost_error_code.value() != boost::system::errc::success)
						{
							...
							if (boost_error_code.value() != boost::asio::error::operation_aborted)        //operation_aborted - означает, что задача была отменена вызовом метода "cancel" у обьекта "resolver_uptr" - поэтому отфильтруем эти "ошибки" - так как на самом деле они не ошбики.
							{
								...
								if (res == false)
								{
									lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error_code, "AsyncReadUntill_handler:async_read_until:" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
								}
								//--------------------------------------------------
							}
						}
						else
						{
							...
							if (Shared_User_Callback_for_Read != 0)
							{
								Shared_User_Callback_for_Read(this, socket_struct_p, (socket_struct_p->Read_struct_.read_untill_.string_buff).c_str(), data_size - socket_struct_p->Read_struct_.read_untill_.user_separator.size());
							}
	...
	void call__async_connect(Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_IP, const int Port_num, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error)> User_Callback_AsyncConnect)
	{

		//------------------------------------------------------------------------------------------------
		boost::asio::ip::tcp::endpoint endpoint(boost::asio::ip::make_address(pointer_to_IP), Port_num);
		//------------------------------------------------------------------------------------------------


		socket_struct_p->socket_.async_connect(endpoint, [this, socket_struct_p, User_Callback_AsyncConnect](const boost::system::error_code& boost_error)
			{

				//Данный лямбда кол-бек ВСЕГДА будет вызываться в том потоке в котором запущен данных обьект класса(при установке при создании класса параметра main_thread) или в нескольких отдельных внутренних потоках (при установке при создании класса параметра othre_threads) Boost Asio - отличных от любого ПОльзовательского потока.


				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error, User_Callback_AsyncConnect]()
				{
						...
						if (boost_error.value() != boost::system::errc::success)
						{
							...
							if (boost_error.value() != boost::asio::error::operation_aborted)            //operation_aborted - означает, что задача была отменена вызовом метода "cancel" у обьекта "resolver_uptr" - поэтому отфильтруем эти "ошибки" - так как на самом деле они не ошбики.
							{
								...
								if (res == false)
								{
									lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error, "call__async_connect:async_connect:" + get__BoostErrorText(boost_error.value()) + ":" + boost_error.message());
								}
								//--------------------------------------------------
							}
						}
						...
						{
							//Значит "соединение" на сокете с удаленным IP адрессом - произошло успешно. Теперь вызвоем функцию записи:


							set__ConnectFlag_value(socket_struct_p, true);    //Установим флаг успешности "соединения".


							//---------------------------------------------------------------------------------------
							const boost::system::error_code boost_error;  //Заглушка для колбека.

							if (Shared_User_Callback_AsyncConnect != 0)
							{
								Shared_User_Callback_AsyncConnect(this, socket_struct_p, Connect_flag::Connect, boost_error);
							}

							if (User_Callback_AsyncConnect != 0)
							{
								User_Callback_AsyncConnect(this, socket_struct_p, Connect_flag::Connect, boost_error);
							}
							//---------------------------------------------------------------------------------------


							//------------------------------------------------------------------------
							for (std::deque<std::function<void()>>::iterator it = socket_struct_p->deque__Tasks_waiting_for_Connection.begin(); it != socket_struct_p->deque__Tasks_waiting_for_Connection.end(); ++it)
							{
								(*it)();
							}

							socket_struct_p->deque__Tasks_waiting_for_Connection.clear();
							//------------------------------------------------------------------------
	...
	void call__async_connect_for_Resolve(Sockets__class::socket_struct* socket_struct_p, const boost::asio::ip::tcp::resolver::results_type& results)
	{

		boost::asio::async_connect(socket_struct_p->socket_, results, [this, socket_struct_p, results](const boost::system::error_code& boost_error, const boost::asio::ip::tcp::endpoint& endpoint)
			{

				//Данный лямбда кол-бек ВСЕГДА будет вызываться в том потоке в котором запущен данных обьект класса(при установке при создании класса параметра main_thread) или в нескольких отдельных внутренних потоках (при установке при создании класса параметра othre_threads) Boost Asio - отличных от любого ПОльзовательского потока.


				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error]()
				{
	...
	void AsyncResolver_handler(Sockets__class::socket_struct* socket_struct_p, const char* pointer_to_Web_Adress, const char* Port_num, std::function<void(BoostAsio__TCP_sender* BoostAsio__TCP_sender_p, Sockets__class::socket_struct* socket_struct_p, const Connect_flag Connect_flag_, const boost::system::error_code& boost_error)> User_Callback_AsyncConnect)
	{

		resolver_uptr.get()->async_resolve(pointer_to_Web_Adress, Port_num, [this, socket_struct_p, User_Callback_AsyncConnect](const boost::system::error_code& boost_error_code, const boost::asio::ip::tcp::resolver::results_type results)
			{
				...
				ThreadPoolTask__uptr.get()->add_task([this, socket_struct_p, boost_error_code, results, User_Callback_AsyncConnect]()
				{
						...
						if (boost_error_code.value() != boost::system::errc::success)
						{
							if (boost_error_code.value() != boost::asio::error::operation_aborted)        //operation_aborted - означает, что задача была отменена вызовом метода "cancel" у обьекта "resolver_uptr" - поэтому отфильтруем эти "ошибки" - так как на самом деле они не ошбики.
							{
								lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error_code, "AsyncResolver_handler:async_resolve:" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
							}
						}
						else
						{
							try
							{
								call__async_connect_for_Resolve(socket_struct_p, results);
							}
							catch (std::exception& e)
							{
								lambda_error(this, socket_struct_p, error_enum::boost_error, boost_error_code, "AsyncResolver_handler:async_resolve:" + get__BoostErrorText(boost_error_code.value()) + ":" + boost_error_code.message());
	...
	const bool CheckSocket_on_Disconnect(Sockets__class::socket_struct* socket_struct_p, const boost::system::error_code& boost_error)
	{
	...
	jump:
		...
		if (check_flag == true)
		{
			if (Shared_User_Callback_AsyncConnect != 0)
			{
				Shared_User_Callback_AsyncConnect(this, socket_struct_p, Connect_flag::Disconnect, boost_error);
			}
		}
